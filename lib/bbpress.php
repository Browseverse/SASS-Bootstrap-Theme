...
/**
 * Functions of SASS Bootstrap theme
 *
 * @package bbPress
 * @subpackage SASS Bootstrap
 * @since SASS Bootstrap 3.0
 */
...
if ( class_exists( 'BBP_Theme_Compat' ) ) :
/**
 * Loads SASS Bootstrap Theme functionality
 * 
 * Usually functions.php contains a few functions wrapped in function_exisits()
 * checks. Since bbp-sass is intended to be used both as a child theme and
 ...
 *
 * @package bbPress
 * @subpackage BBP_SASS_Bootstrap_bbPress
 */
class BBP_SASS_Bootstrap_bbPress extends BBP_Theme_Compat {

  /** Functions *************************************************************/

  /**
   * The main bbPress (SASS Bootstrap) Loader
   *
   * @since bbPress (r3277)
   *
   * @uses BBP_SASS_Bootstrap_bbPress::setup_globals()
   * @uses BBP_SASS_Bootstrap_bbPress::setup_actions()
   */
  ...
  private function setup_globals() {
    ...
    $this->name = 'bbPress (SASS Bootstrap)';

    // Version of theme in YYYMMDD format
    $this->version = '20120321';

    // Setup the theme path
    $this->dir = $bbp->themes_dir . '/sass-bootstrap';

    // Setup the theme URL
    $this->url = $bbp->themes_url . '/sass-bootstrap';
  }
  ...
  public function enqueue_styles() {
		...
		if( is_rtl() ) // bbPress right-to-left styles
			wp_enqueue_style( 'sass-bbp-bbpress-rtl', get_template_directory_uri() . '/stylesheets/bbpress-rtl.css', 'sassbs-bbpress-rtl', $this->version, 'screen' );
  ...
  public function enqueue_scripts() {
    ...
    if ( bbp_is_single_topic() )
      wp_enqueue_script( 'bbp_topic', get_stylesheet_directory_uri() . '/js/topic.js', array( 'wp-lists' ), $this->version, true );
...
/**
 * Instantiate a new BBP_SASS_Bootstrap_bbPress class inside the $bbp global. It is
 * responsible for hooking itself into WordPress where apprpriate.
 */
if ( 'bbPress' == get_class( $bbp ) ) {
  $bbp->theme_compat->theme = new BBP_SASS_Bootstrap_bbPress();
}
